...
`collect-results` 只登记 PNG、JPEG、WebP 或 GIF。`output_manifest.image_count` 必须是与 `images` 数组长度完全相等的整数，并且每个 `images` 条目都必须是对象；结构或数量不一致时返回 `comfyui_output_manifest_invalid`，且不得下载、写入或登记任何产物。下载内容必须同时通过格式签名、完整尾标和扩展名匹配检查；HTML 错误页、空内容、截断文件、伪造扩展名或其他格式均返回 `comfyui_output_fetch_failed`，不得写入 Project artifacts 或 Evidence。多图结果按一个登记批次处理：任一后续图片下载、校验、写入或登记失败时，必须删除本批次已经写入的文件；如果清理本身失败，则升级为 `comfyui_output_rollback_failed`，不得把部分结果报告为成功。这一道门只验证产物清单一致性、产物类型与基本完整性，不把模拟回环测试描述成真实桌面 ComfyUI 验收。
