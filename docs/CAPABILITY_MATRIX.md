...
| AutoCAD / DXF headless | planning, execution, validation, evidence, cleanup | `autocad_dxf.validate_cad_plan`, `autocad_dxf.summarize_plan`, DXF dry-run | guarded `write_dxf` with `confirm_write=true` | richer CAD entity schema | 回读 DXF 的单位、图层、几何和文字内容指纹必须匹配批准 plan，再生成保留实体 ID、DXF 图层和类型元数据的白底黑线 SVG 预览；manifest 记录内容与产物 hash、实体数、路径/图层数和外部引用检查；无需桌面启动 | only `examples/cad/output` | Yes | No | path cannot escape sandbox output root；DXF/SVG/manifest 任一失败则回滚当前批次 |
