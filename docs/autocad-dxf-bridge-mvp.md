...
- 新鲜普通 staging 会返回非终态 `generation_in_progress` 和脱敏的 `retry_after_seconds`，方便调用方等待重试；目录、符号链接或最终文件仍作为硬冲突处理。
...
- 失败结果只返回脱敏的 `failure.type`，并用 `rollback.complete`、清理计数和 `same_output_retry_allowed` 明确区分“可安全原名重试”与“回滚不完整”；不回传异常原文或本地路径。
...
| `ellipse` | `center`, `major_axis`, `ratio`, `layer`；生成完整椭圆 |
...
`ellipse` 当前固定生成 XY 平面的完整椭圆；椭圆弧和 3D extrusion 需要额外的参数方向与包围盒验证后再开放。
...
2. 增加更多实体：dimension、mtext、椭圆弧，以及带孔或图案的复杂 hatch。
